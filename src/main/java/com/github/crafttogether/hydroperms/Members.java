...
public class Members {
    private final static File folder = Plugin.getInstance().getDataFolder();

    public static JSONArray get() {
        try {
            final File members = new File(Plugin.getFilePath());
    ...
    public static void update(JSONArray json) {
        try {
            final Writer toilet = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(Plugin.getFilePath()), StandardCharsets.UTF_8));
